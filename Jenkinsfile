         ...
         stage('Deploy') {
            steps {
                script {
                    ...
                    // def auth = "tomcat-sc:ani#1234" 

                    def tomcatBaseUrl = 'http://174.129.249.22:8080'  
                    def warFileName = 'hello-world.war'  
                  
                    def deployUrl = "${tomcatBaseUrl}/manager/text/deploy?path=/&war=file:hello-world.war"
                    def curlCommand = "curl -T target/hello-world.war --user tomcat-sc:ani#1234 ${deployUrl}"
